...
Application::Application() :oPlane(), rotateX(glm::mat4(1.0f)), rotateY(glm::mat4(1.0f)), views(0) {
...
void Application::setup() {
	...
	//Textura
	std::string strVertexShaderTexture = loadTextFile("Shaders/passThru.v");
	std::string strFragmentShaderTexture = loadTextFile("Shaders/passThru.f");
	...
	//Textura
	InitializeProgram(oPlane.shader, strVertexShaderTexture, strFragmentShaderTexture);
...
{
	std::cout << xpos << ", " << ypos << "\n";
	rotateX = glm::rotate(glm::mat4(1.0f), (float)glm::radians(ypos), glm::vec3(1.0f, 0.0f, 0.0f));
	rotateY = glm::rotate(glm::mat4(1.0f), (float)glm::radians(xpos), glm::vec3(0.0f, 1.0f, 0.0f));
}
